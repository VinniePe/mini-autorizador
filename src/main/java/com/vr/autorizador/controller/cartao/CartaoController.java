...
public class CartaoController {
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "201", description = "Created", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = CartaoNovo.class)))),
			@ApiResponse(responseCode = "422", description = "Unprocessable Entity", content = @Content(schema = @Schema(hidden = true))), })
	@PostMapping(value = "/")
	public ResponseEntity<List<CartaoNovo>> criarLista(@RequestBody List<@Valid CartaoNovo> cartaoNovo) {
		try {
			return new ResponseEntity<>(cartaoService.criar(cartaoNovo), CREATED);
		} catch (Exception e) {
			return new ResponseEntity<>(null, UNPROCESSABLE_ENTITY);
